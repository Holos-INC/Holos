import {
  StatusKanbanDTO,
  StatusKanbanWithCommissionsDTO,
...
export async function fetchStatusesWithCommissions(
  token: string
): Promise<StatusWithCommissions[]> {
  const res = await api.get("/status-kanban-order", {
...
export async function addStatusColumn(
  dto: StatusKanbanCreateDTO,
  token: string
) {
  await api.post("/status-kanban-order", dto, {
    headers: { Authorization: `Bearer ${token}` },
  });
}

export async function updateStatusColumn(
  dto: StatusKanbanUpdateDTO,
  token: string
) {
  await api.put("/status-kanban-order/update", dto, {
    headers: { Authorization: `Bearer ${token}` },
  });
}

export async function moveCommissionForward(id: number, token: string) {
  await api.put(`/status-kanban-order/${id}/next`, null, {
    headers: { Authorization: `Bearer ${token}` },
  });
}

export async function moveCommissionBack(id: number, token: string) {
  await api.put(`/status-kanban-order/${id}/previous`, null, {
    headers: { Authorization: `Bearer ${token}` },
  });
}
