...
public interface CommisionRepository extends JpaRepository<Commision, Long>{
    ...
    @Query("SELECT new com.HolosINC.Holos.commision.DTOs.CommissionDTO(c.id, c.name, c.description, c.price, c.status, c.paymentArrangement, c.milestoneDate, c.artist.baseUser.username, c.client.baseUser.username, c.image, c.artist.baseUser.imageProfile, c.client.baseUser.imageProfile)" +
    ...
    @Query(" SELECT new com.HolosINC.Holos.commision.DTOs.ClientCommissionDTO(c.image, c.name, c.artist.baseUser.username, c.statusKanbanOrder.order,0, c.artist.baseUser.imageProfile, c.client.baseUser.imageProfile)"+
    ...
    Optional<Commision> findFirstByStatusOrderByCreatedDateAsc(StatusCommision status);
